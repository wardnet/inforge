# VM secret bootstrap uses a centralized inforge escrow plus SOPS/age re-encryption

Secret values in a service manifest are encrypted at provision time with SOPS/age to a key `K`.
inforge mints `K` plus a one-time token `T`, registers `K` under `T` with the **inforge escrow
service** (a Cloudflare Worker at `escrow-worker/`, operated by the inforge project), and writes
the escrow URL + `T` into a `bootstrap.yaml`. At first boot the VM redeems `T`→`K`, decrypts the
values, and re-encrypts them to its own host SSH key (age supports ssh-ed25519), then deletes
`bootstrap.yaml`. inforge both provides and operates the escrow.

**Update (inforge 1.0):** The escrow was previously external (owned in `wardnet-infrastructure`).
It is now a first-class part of inforge (`escrow-worker/`). The service is open to any GitHub
