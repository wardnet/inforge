...
`inforge` and `inforge-bootstrap` must agree on byte-for-byte: `RuntimeDir`,
`RuntimeSubdir`, and `UnitName`. Never re-derive these strings inline — any
drift causes the deploy-side and the bootstrap binary to disagree on the
projected path.
...
`internal/hostpaths` is intentionally stdlib-only so `inforge-bootstrap` can
