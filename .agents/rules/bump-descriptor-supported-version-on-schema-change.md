...
Any addition, removal, or rename of a field in `internal/bootstrapper.Descriptor`
or `internal/bootstrapper.Deployment` is a breaking change for older bootstrapper
...
bootstrapper versions can silently misread or reject descriptors.

## Applies to

`internal/bootstrapper/descriptor.go` — the `Descriptor` and `Deployment` structs
...
type Deployment struct {
    ...
    HostID string `yaml:"host_id"`  // new field — older bootstrapper rejects it silently
}
