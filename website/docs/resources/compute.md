...
must be a single-line OpenSSH public key (`<type> <base64> [comment]`, with no `'` in the comment).
Both are substituted into the root-run first-boot script, so `inforge validate` rejects a name outside
that charset and the deploy fails fast on a malformed key rather than booting a host with an account
nobody asked for.
